...
class FGANet(nn.Module):
    ...
    def __init__(self,
        ...
        self.features = nn.Sequential(
            ...
            nn.BatchNorm1d(conv_channels*2 ),
            nn.Conv1d(conv_channels*2, conv_channels * 4, kernel_size, stride),
            nn.ReLU(),
            nn.Dropout(p=0.1),
        ...
        #                                 nn.ReLU(), nn.Dropout(p=0.5),nn.Linear(64, num_output))
        # self.classifier = nn.Sequential(nn.Linear(self.num_dense_input, 128),
        #                                 nn.ReLU(),nn.Dropout(p=0.5),  # Dropout added here
